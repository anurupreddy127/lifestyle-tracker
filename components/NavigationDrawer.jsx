...
export default function NavigationDrawer({ isOpen, onClose }) {
  ...
  return (
    <>
      ...
      {/* Drawer panel */}
      <div
        className={`fixed left-0 top-0 bottom-0 w-[280px] bg-bg-nav z-[60] shadow-2xl transition-transform duration-300 ease-out flex flex-col ${
      ...
      >
        ...
        <div className="p-5 pb-4">
          ...
          <p className="font-bold text-text-primary text-sm">{user?.email || 'User'}</p>
          <p className="text-xs text-text-secondary mt-0.5">Lifestyle Tracker</p>
        </div>
        ...
        {/* Workspace switcher */}
        <div className="p-3">
          <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest px-3 mb-2">Workspace</p>
          <Link
            href="/gym"
            onClick={onClose}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
              !isFinance ? 'bg-accent/15 text-accent font-semibold' : 'text-text-secondary hover:bg-white/5'
            }`}
          >
          ...
          <Link
            ...
            onClick={onClose}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
              isFinance ? 'bg-finance/15 text-finance font-semibold' : 'text-text-secondary hover:bg-white/5'
            }`}
          >
        ...
        <div className="p-3 flex-1">
          <p className="text-[10px] font-bold text-text-secondary uppercase tracking-widest px-3 mb-2">Navigation</p>
          {navLinks.map((link) => {
            const active = pathname === link.href
            const color = isFinance ? 'text-finance' : 'text-accent'
            const bg = isFinance ? 'bg-finance/15' : 'bg-accent/15'
            return (
              <Link
                key={link.href}
                href={link.href}
                onClick={onClose}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                  active ? `${bg} ${color} font-semibold` : 'text-text-secondary hover:bg-white/5'
                }`}
              >
        ...
        <div className="p-3">
          <Link
            href="/profile"
            onClick={onClose}
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-text-secondary hover:bg-white/5 transition-colors"
          >
