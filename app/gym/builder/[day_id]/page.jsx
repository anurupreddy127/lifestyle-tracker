  ...
  return (
    <div className="px-4 pt-2 pb-4">
          ...
          {selectedExercises.map((ex, index) => (
            <div key={ex.exercise_id} className="bg-dark-card border border-dark-border rounded-2xl p-4 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-dark-muted mt-0.5">{ex.target_sets} sets × {ex.target_reps} reps</p>
              </div>
      ...
      <BottomSheet isOpen={showPicker} onClose={() => setShowPicker(false)} title="Pick Exercise">
        ...
        <div className="flex flex-col gap-2 max-h-[40vh] overflow-y-auto">
          {filteredExercises.length === 0 ? (
            <p className="text-sm text-dark-muted text-center py-4">No matching exercises.</p>
          ) : (
      ...
      <BottomSheet isOpen={showTargetPrompt} onClose={() => setShowTargetPrompt(false)} title="Set Targets">
        {pendingExercise && <p className="text-sm text-dark-muted mb-4">{pendingExercise.name}</p>}
        ...
        <button onClick={handleConfirmTarget} className="bg-primary text-white font-semibold rounded-xl py-3.5 w-full text-base active:bg-primary/90">
          Confirm
        </button>
      </BottomSheet>
    </div>
