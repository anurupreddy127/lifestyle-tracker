  ...
  return (
    <div className="px-4 pt-2 pb-4">
          ...
          {selectedExercises.map((ex, index) => (
            <div key={ex.exercise_id} className="glass rounded-2xl p-4 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 mt-0.5">{ex.target_sets} sets × {ex.target_reps} reps</p>
              </div>
      ...
      <BottomSheet isOpen={showPicker} onClose={() => setShowPicker(false)} title="Pick Exercise">
        ...
        <div className="flex flex-col gap-2 max-h-[40vh] overflow-y-auto">
          {filteredExercises.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-4">No matching exercises.</p>
          ) : (
      ...
      <BottomSheet isOpen={showTargetPrompt} onClose={() => setShowTargetPrompt(false)} title="Set Targets">
        {pendingExercise && <p className="text-sm text-slate-500 mb-4">{pendingExercise.name}</p>}
