...
export default function EditDayBuilder() {
  ...
  return (
    <div className="px-4 pt-2 pb-4">
          ...
          {selectedExercises.map((ex, index) => (
            <div key={ex.exercise_id} className="bg-bg-card border border-border rounded-2xl p-4 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-text-secondary mt-0.5">{ex.target_sets} sets × {ex.target_reps} reps</p>
              </div>
      ...
      <BottomSheet isOpen={showPicker} onClose={() => setShowPicker(false)} title="Pick Exercise">
        ...
        <div className="flex flex-col gap-2 max-h-[40vh] overflow-y-auto">
          {filteredExercises.length === 0 ? (
            <p className="text-sm text-text-secondary text-center py-4">No matching exercises.</p>
          ) : (
      ...
      <BottomSheet isOpen={showTargetPrompt} onClose={() => setShowTargetPrompt(false)} title="Set Targets">
        {pendingExercise && <p className="text-sm text-text-secondary mb-4">{pendingExercise.name}</p>}
        <div className="flex gap-4 mb-6">
          <div className="flex-1">
            <label className="text-sm font-medium text-text-secondary mb-1.5 block">Sets</label>
            <input type="text" inputMode="numeric" pattern="[0-9]*" value={tempSets} onChange={(e) => setTempSets(e.target.value)}
          ...
          <div className="flex-1">
            ...
            <input type="text" inputMode="numeric" pattern="[0-9]*" value={tempReps} onChange={(e) => setTempReps(e.target.value)}
              className="bg-bg-input border border-border rounded-xl px-4 py-3 text-text-primary text-lg text-center focus:outline-none focus:ring-2 focus:ring-accent/30 w-full font-bold" />
          </div>
        </div>
        <button onClick={handleConfirmTarget} className="bg-accent text-white font-semibold rounded-xl py-3.5 w-full text-base active:bg-accent/90">
          Confirm
        </button>
      </BottomSheet>
    </div>
